...
class Table{
    ...
    throwAwayCards = ()=> {
        for (let i = 0; i < 4; i++){
            for (let j = 0; j < 4; j++){
                if (j != i){
                    if ( this.content[i].length > 0 && this.content[j].length > 0 &&
                        this.content[i][this.content[i].length-1].suit == this.content[j][this.content[j].length-1].suit ){
                        if (this.content[i][this.content[i].length-1].value < this.content[j][this.content[j].length-1].value){
                            this.content[i].pop();
                        }
...
for (let i = 0; i < 100000; i++) {
    ...
    do {
        table.giveCards(deck.drawCard(), deck.drawCard(), deck.drawCard(), deck.drawCard());
        for (let i = 0; i < 4; i++){
...
console.log(`The winning percentage of this bot is: ${percentage}%`);